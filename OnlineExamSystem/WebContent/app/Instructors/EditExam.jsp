<%@ page import="domain.Exam, mappers.ExamMapper, domain.Question, mappers.QuestionMapper, domain.Choice, mappers.ChoiceMapper, java.util.List" %>
...
<body onload="poistiveNumberValidation()">
	...
	%>
	<form id="AddExamForm" name="AddExamForm" action="${pageContext.request.contextPath}/editExam" method="post">
		<div id="ExamHeader">
			<label>Exam Name: </label>
			<input type="text" name="examName" id="ExamNameInput" value="<%= exam.getName() %>">
			<input type="hidden" name="subjectCode" value="<%= exam.getSubjectCode() %>">
			<input type="hidden" name="examId" value="<%= exam.getExamID() %>">
			<input type="hidden" id="AmountOfQuestionInput" name="amountOfQuestions" value="<%=questions.size()%>">
			<input type="hidden" id="isPublishedInput" name="isPublished" value="<%= exam.getIsPublishedVar() %>">
			<input type="hidden" id="modifiedByInput" name="modifiedBy" value="<%=(String) session.getAttribute("username")%>">
			<input type="hidden" id="verInput" name="ver" value="<%=exam.getVer()%>">
			
			<label>Time Allowed (Minutes):</label>
			<input type="number" id="timeAllowedInput" name="timeAllowed" min="0" value="<%= exam.getTimeAllowed() %>">
		</div>
		<div id="QuestionBody">
			...
			<div class ="Question">
				<div class="QuestionHeader">
					<ul>
						<li class="QuestionIndexLabel">Question <%= question.getQuestionIndex() %></li>
						<li style="float:right"><%= question.getMarks() %> pts</li>
					</ul>
				</div>
				<div class="QuestionContent"><%= question.getQuestionText() %></div>
				<% if (question.getQuestionType().equals("MC")){ %>
				<div class="QuestionChoices">
					<% for (Choice choice : ChoiceMapper.getAllChoicesFromQuestion(question.getQuestionID()))  { %>
						<div>
							<label id="choiceNumberText<%=choice.getChoiceIndex()%>">Choice <%=choice.getChoiceIndex()%></label>
							<label id="choiceContent<%=choice.getChoiceIndex()%>"><%=choice.getChoiceText()%></label>
						</div>
					<% } //end for loop %>
				</div>
				<% 	} //end if statement %>
				<div class="QuestionOptions" style="display: block;">
					<ul>
						<li>
							<button type="button" class="editButton" onclick="editQuestion(this)">Edit</button>
						</li>
						<li>
							<button type="button" class="deleteButton" onclick="deleteQuestion(this)">Delete</button>
						</li>
					</ul>
				</div>
				<div style="display:none" id="hiddenForm<%=question.getQuestionIndex()%>">
					<input type="hidden" name="examId<%=question.getQuestionIndex()%>" value="<%=exam.getExamID()%>">
					<input type="hidden" name="questionIndex<%=question.getQuestionIndex()%>" value="<%=question.getQuestionIndex()%>">
					<input type="hidden" name="questionText<%=question.getQuestionIndex()%>" value="<%=question.getQuestionText()%>">
					<input type="hidden" name="points<%=question.getQuestionIndex()%>" value="<%=question.getMarks()%>">
					<input type="hidden" name="questionType<%=question.getQuestionIndex()%>" value="<%=question.getQuestionType()%>">
					<%  if (question.getQuestionType().equals("MC")){
						List<Choice> choices = ChoiceMapper.getAllChoicesFromQuestion(question.getQuestionID());
							for (Choice choice : choices)  { %>
					<input type="hidden" name="q<%=question.getQuestionIndex()%>Choice<%=choice.getChoiceIndex()%>" id="q<%=question.getQuestionIndex()%>Choice<%=choice.getChoiceIndex()%>" value="<%=choice.getChoiceText()%>">
					<input type="hidden" name="questionId<%=question.getQuestionIndex()%>Choice<%=choice.getChoiceIndex()%>" id="questionId<%=question.getQuestionIndex()%>Choice<%=choice.getChoiceIndex()%>" value="<%=question.getQuestionID()%>">
					<% 		} //end for loop %>
					<input type="hidden" name="q<%=question.getQuestionIndex()%>AmountOfChoices" id="q<%=question.getQuestionIndex()%>AmountOfChoices" value="<%=choices.size()%>">
					<% } //end if statement %>
					<input type="hidden" name="questionId<%=question.getQuestionIndex()%>" value="<%=question.getQuestionID()%>">
				</div> 
			</div>
			<% } // end for loop %>
		</div>
		<div style="display:none" id="DeletedQuestionsDiv">
			<input type="hidden" id="AmountOfDeletedQuestionInput" name="amountOfDeletedQuestions" value="0">
			<input type="hidden" id="DeletedQuestionInput" name="DeletedQuestions" value="">
		</div>
	</form>
	...
	<div class="Bottom" id="Bottom">
		...
		<ul>
			<li>
				<button id="saveExamBtn" type="button" onclick="saveExam()">Save</button>
			</li>
...
<script type="text/javascript">
	...
	function addQuestion() {
		if (document.getElementById("PtsInput").value == ""){
			document.getElementById("PtsInput").value = "0"
		}
		...
		examIdInput.setAttribute("value", "<%=exam.getExamID()%>");
		...
		var newQuestionIdInput = document.createElement("input");
		newQuestionIdInput.setAttribute("type", "hidden");
		newQuestionIdInput.setAttribute("name", "questionId" + questionIndex);
		newQuestionIdInput.setAttribute("value", "-1");
		...
		hiddenFormDiv.appendChild(newQuestionIdInput);		
	...
	function deleteQuestion(self){
		...
		question = removeTextNodes(question);
		...
		for (var i = deletedIndex; i < questionsIndexLabels.length; i++) {
			questions[i] = removeTextNodes(questions[i]);
			...
			if (questions[i].childNodes[2].className === "QuestionChoices"){
				for (j = 5; j < questions[i].lastChild.childNodes.length - 2; j += 2){
				...
				questions[i].lastChild.childNodes[questions[i].lastChild.childNodes.length-2].name = replaceChoiceAmountStr(questions[i].lastChild.childNodes[questions[i].lastChild.childNodes.length-2].name , i);
				questions[i].lastChild.childNodes[questions[i].lastChild.childNodes.length-2].id = replaceChoiceAmountStr(questions[i].lastChild.childNodes[questions[i].lastChild.childNodes.length-2].id , i);
			}
			questions[i].lastChild.lastChild.name = questions[i].lastChild.lastChild.name.substring(0, 10) + i
		};
		
		//add questionId to deleted questions
		if (question.lastChild.lastChild.value !== "-1"){
			document.getElementById("AmountOfDeletedQuestionInput").value ++;
			deletedQuestionsInput = document.getElementById("DeletedQuestionInput");
			deletedQuestionsInput.value = deletedQuestionsInput.value + question.lastChild.lastChild.value + ","
		}
	...
	function removeTextNodes(question){
		//remove text nodes
		for(var i = 0; i < question.childNodes.length; i++){
			if (question.childNodes[i].nodeType === 3){
				question.removeChild(question.childNodes[i]);
				i--;
			}
		}
		
		for(var i = 0; i < question.childNodes[0].childNodes.length; i++){
			if (question.childNodes[0].childNodes[i].nodeType === 3){
				question.childNodes[0].removeChild(question.childNodes[0].childNodes[i]);
				i--;
			}
		}
		
		for(var i = 0; i < question.childNodes[0].childNodes[0].childNodes.length; i++){
			if (question.childNodes[0].childNodes[0].childNodes[i].nodeType === 3){
				question.childNodes[0].childNodes[0].removeChild(question.childNodes[0].childNodes[0].childNodes[i]);
				i--;
			}
		}
		
		if (question.childNodes[2].className === "QuestionChoices"){
			for(var i = 0; i < question.childNodes[4].childNodes.length; i++){
				if (question.childNodes[4].childNodes[i].nodeType === 3){
					question.childNodes[4].removeChild(question.childNodes[4].childNodes[i]);
					i--;
				}
			}
		} else {
			for(var i = 0; i < question.childNodes[3].childNodes.length; i++){
				if (question.childNodes[3].childNodes[i].nodeType === 3){
					question.childNodes[3].removeChild(question.childNodes[3].childNodes[i]);
					i--;
				}
			}
		}
	...
	function UpdateQuestion(self){
		...
		var updateQuestionIdInput = document.createElement("input");
		updateQuestionIdInput.setAttribute("type", "hidden");
		updateQuestionIdInput.setAttribute("name", "questionId" + questionIndex);
		updateQuestionIdInput.setAttribute("value", document.getElementById("hiddenForm" + questionIndex).lastChild.value);
		...
		question.appendChild(questionOptions);
		hiddenFormDiv.appendChild(updateQuestionIdInput);
		question.appendChild(hiddenFormDiv);
	    ...
	    $.ajax({
	        ...
	        success: function(response){
	        	if(response.localeCompare('success') == 1){
	        		...
	        		alert(response);
	        		form.action = form.action.substring(0, form.action.length-8) + 'createExam'
	        		document.getElementById("saveExamBtn").textContent = 'Save as a new copy';
